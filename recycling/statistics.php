...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['username'])) {
        ...
        $sql = "SELECT plastic, glass, aluminium, paper, achievements FROM users WHERE username='$user'";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            echo json_encode(array("status" => "success", "plastic" => $row["plastic"], "glass" => $row["glass"], "aluminium" => $row["aluminium"], "paper" => $row["paper"], "achievements" => $row["achievements"]));
        } else {
